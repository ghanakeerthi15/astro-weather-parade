  ...
  const fetchWeatherData = async (data: { city: string; eventName: string; date: string }) => {
    ...
    try {
      ...
      const weatherResponse = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&a46e7d640088dc4cdb814db3a21104e0`
    );
      
      const forecastResponse = await fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&a46e7d640088dc4cdb814db3a21104e0`
      );
